...
<ul class="pagination">	
	<c:if test="${pagedListHolder.pageCount>1}">
		...
		<li ><span class="paginDots">...</span></li>
		</c:if>
		<c:forEach begin="${pagedListHolder.firstLinkedPage}" 
		        end="${pagedListHolder.lastLinkedPage}" var="i" >
		    <c:choose>
		     	<c:when test="${pagedListHolder.page==i}">
		     		<li class="active"><a href="<%=StringUtils.replace(pagedLink, "~", String.valueOf(pagedListHolder.getPage()))%>">${i+1}</a></li>
		     	</c:when> 
		     	<c:otherwise>
		...
		<c:if test="${pagedListHolder.firstLinkedPage< pagedListHolder.pageCount-2}">
				<li ><span class="paginDots">...</span></li>
		</c:if>
